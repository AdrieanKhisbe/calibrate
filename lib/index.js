...
internals.decorate = module.exports.decorate = function decorate(server, options, next) {
    ...
    const validation = Joi.validate(options, internals.optionSchema);
 ...
 */
internals.responseFactory = function responseFactory(_globalOptions) {

    const globalOptions = _globalOptions || {};
    const dataHandler = globalOptions.dataHandler || internals.defaults.dataHandler;
    const notFoundHandler = globalOptions.notFoundHandler || internals.defaults.notFoundHandler;
